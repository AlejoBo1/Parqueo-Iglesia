...
// Formulario de Registro de Pagos Adelantados
document.getElementById('form-registro-pago').onsubmit = async (e) => {
    e.preventDefault();
    const pId = parseInt(document.getElementById('pago-puesto-num').value);
    const p = parqueoData.find(x => x.id === pId);
    
    if (!p || !p.ocupado) return alert("El puesto seleccionado está vacío.");
    ...
    const exito = await enviarAGoogle(pago);
...
function updateStats() {
    ...
    const elOcupados = document.getElementById('stat-ocupados');
    const elLibres = document.getElementById('stat-libres');
    
    if(elOcupados) elOcupados.innerText = oc;
    if(elLibres) elLibres.innerText = 100 - oc;
}

function actualizarListaPagos() {
    const el = document.getElementById('lista-historial-pagos');
    if(el) {
        el.innerHTML = historialPagos.map(p => `<li>Puesto ${p.puestoId}: $${p.monto}</li>`).reverse().join('');
    }
}